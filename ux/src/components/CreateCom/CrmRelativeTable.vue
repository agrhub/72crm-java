<template>
  <div class="cr-body-content">
    <flexbox class="content-header">
      <div v-if="!isRelationShow">Lọc nhanh:</div>
      <el-dropdown
        v-if="!isRelationShow"
        trigger="click"
        @command="handleTypeDrop">
        <flexbox>
          <div>{{ sceneInfo ? sceneInfo.name : 'Vui lòng chọn' }}</div>
          <i
  ...
  methods: {
      ...
      crmSceneIndex({
        ...
        .then(res => {
          ...
          if (this.scenesList.length == 0) {
            this.scenesList.push({ sceneId: '', name: 'Tất cả' })
    ...
    getDefaultField() {
      if (this.crmType === 'leads') {
        return [
          { name: 'Tên khách hàng', field: 'name', formType: 'leads' },
        ...
        return [
          ...
          { name: 'Số bàn', field: 'telephone', formType: 'text' },
          { name: 'Người ra quyết định chính', field: 'key_decision_marker', formType: 'text' },
        ...
        return [
          ...
          { name: 'Trạng thái', field: 'on_or_off_the_shelf', formType: 'text' }
        ]
      }
    },
    /** 获取列表数据 */
