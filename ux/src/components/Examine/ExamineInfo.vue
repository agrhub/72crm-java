    ...
    <flexbox
      ...
      <el-popover
        ...
        <el-button
          ...
          type="text">Xem lịch sử phê duyệt</el-button>
      </el-popover>
      <div style="min-height: 40px;">
        <el-button
          v-if="examineInfo.isRecheck==1"
          class="flow-button white"
          @click="examineHandle(4)">Rút lại đánh giá</el-button>
        <el-button
          v-if="examineInfo.isCheck==1"
          class="flow-button red"
          @click="examineHandle(2)">Từ chối</el-button>
        <el-button
          v-if="examineInfo.isCheck==1"
          class="flow-button blue"
          @click="examineHandle(1)">Thông qua</el-button>
      </div>
    </flexbox>
    <flexbox
...
export default {
  ...
  filters: {
    ...
    detailName: function(data) {
      if (data.stepType == 2) {
        return data.userList.length + 'Họ tên và chữ ký'
        ...
        return 'Người giám sát cấp trên của người phê duyệt'
      } else if (data.stepType == 5) {
        return 'Người đề xuất'
      }
    },
    stepName: function(index) {
      return 'Đầu tiên' + Nzhcn.encodeS(index) + 'cấp'
    }
  },
  props: {
    examineType: {
