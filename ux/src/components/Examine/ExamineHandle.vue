<template>
  <el-dialog
    ...
    <div
      ...
      <flexbox class="handle-item">
        ...
        <div
          ...
          @click.native="handleType=1">Kết thúc đánh giá</div>
      </flexbox>
      <flexbox
        id="selectUser"
        class="handle-item">
        ...
        <xh-user-cell
          class="select-user"
          placeholder="Chọn người phê duyệt tiếp theo"
    ...
    <div
      slot="footer"
      class="dialog-footer">
      <el-button @click="handleClick('cancel')">Hủy</el-button>
      <el-button
        type="primary"
        @click="handleClick('confirm')">Xác nhận</el-button>
    </div>
  </el-dialog>
</template>
  ...
  computed: {
    title() {
      if (this.status == 1) {
        ...
        return 'Phê duyệt bị từ chối'
      } else if (this.status == 4) {
        return 'Rút lại phê duyệt'
      }
      return ''
    },
    placeholder() {
      // 1 审核通过 2 审核拒绝 4 已撤回
      if (this.status == 1) {
        return 'Vui lòng nhập nhận xét phê duyệt (tùy chọn)'
      } else if (this.status == 2) {
        return 'Vui lòng nhập nhận xét phê duyệt (bắt buộc)'
      } else if (this.status == 4) {
        return 'Vui lòng nhập lý do rút phê duyệt (bắt buộc)'
      }
      return ''
    }
  },
