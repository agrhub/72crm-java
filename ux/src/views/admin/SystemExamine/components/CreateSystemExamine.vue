          ...
          <div style="padding: 0 20px; font-size: 12px;">
            ...
            <div class="examine-items">
              <flexbox
                ...
                :key="index"
                class="examine-item">
                <div class="examine-item-name">Gồm {{ (index+1) | numberToZh }} cấp</div>
                <el-select
                  v-model="item.type"
    ...
    title() {
      if (this.handle.type === 'examineflow') {
        if (this.handle.action === 'save') {
          return 'Quy trình phê duyệt mới'
        } else if (this.handle.action === 'update') {
          return 'Chỉnh sửa quy trình phê duyệt'
        }
      }
      return ''
    }
  ...
  methods: {
    ...
    getField() {
      ...
      field.push({
        ...
        isNull: 0,
        name: 'Phân loại',
        setting: [{ name: 'Hợp đồng', value: 1 }, { name: 'Thanh toán', value: 2 }],
      ...
      field.push({
        ...
        name: 'Mô tả',
        setting: [],
        inputTips: 'Vui lòng điền các lưu ý liên quan để nhân viên tiện kiểm tra khi nộp đơn, và giới hạn đầu vào 2000 từ',
