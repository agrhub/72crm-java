<template>
  <flexbox
    ...
    orient="vertical"
    align="stretch">
    <div class="title">{{ 'Chỉnh sửa '+getTitle()+' thông tin' }}</div>
    <el-container class="wrapper">
      <el-aside class="left">
        <div class="mini-title">Thông tin</div>
        <ul>
          <draggable
            :list="fieldList"
      ...
      <el-container
        ...
        <el-header>
          ...
          <el-button
            style="padding: 8px 22px;border-radius:2px;"
            @click="handleCancel">Hủy</el-button>
        ...
        <el-main>
          ...
          <p
            ...
            class="no-list">Nhấp hoặc kéo từ bên trái để thêm trường thông tin</p>
        </el-main>
      </el-container>
      <el-aside
        class="right"
        width="310px">
        <div class="mini-title">Thuộc tính thông tin</div>
        <field-info
          v-if="form"
          :field="form"
...
export default {
  ...
  methods: {
    ...
    handleDelete(item, index) {
      this.$confirm('Bạn có chắc chắn xóa trường thông tin tùy chỉnh này không?', 'Thông tin', {
    ...
    handleSave() {
      ...
      for (let index = 0; index < tempFieldArr.length; index++) {
        ...
        if (!item.name) {
          ...
          this.$message({
            type: 'error',
            message: 'Có ' + (index + 1) + ' trường thông tin bị trống'
          })
          break
