  ...
  <div
    ...
    <div class="handle-bar">
      ...
      <el-select
        ...
        placeholder="Chọn">
        <el-option
          v-for="item in [{label:'Ngày bắt đầu', value:1},{label:'Ngày kết thúc', value:2}]"
          :key="item.value"
          :label="item.label"
          :value="item.value"/>
      </el-select>
      ...
      <div
        ...
        <el-button
          type="primary"
          @click.native="handleClick('export')">Xuất báo cáo</el-button>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      fieldList: [
        { field: 'name', name: '' },
        { field: 'yeartarget', name: 'Mục tiêu cả năm' },
  ...
  methods: {
    ...
    getSubTotalModel() {
      var initModel = {
        name: 'Tổng các mục tiêu dành cho cấp dưới trực tiếp',
        january: '0.00',
        february: '0.00',
        march: '0.00',
        april: '0.00',
    ...
    handleClick(type) {
      if (type == 'search') {
        ...
        const name = `${this.dateSelect} 年${
          { department: 'Phòng ban', user: 'Nhân viên' }[this.tabType]
        }.xlsx`
        FileSaver.saveAs(
