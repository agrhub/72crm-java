...
export default {
  ...
  computed: {
    ...
    fieldList() {
      const temps = [
        { label: 'Phạm vi áp dụng', field: 'range' },
        {
          label: {
            1: 'Số lượng khách hàng tối đa',
            2: 'Số lượng khách hàng từ chối tối đa'
          }[this.types],
          field: 'customerNum'
        }
      ]
      ...
      if (this.types == 1) {
        temps.push({
          label: {
            1: 'Số lượng khách hàng đã chốt đơn',
            2: 'Số lượng khách hàng hủy đơn'
          }[this.types],
          field: 'customerDeal'
        })
      }
  ...
  methods: {
    ...
    fieldFormatter(row, column) {
      if (column.property == 'customerDeal') {
        return row.customerDeal == 1 ? 'Chốt đơn' : 'Hủy đơn'
      } // 0 不占用 1 占用
     ...
     */
    handleDelete(scope) {
      this.$confirm('Xác nhận xóa?', 'Thông tin', {
