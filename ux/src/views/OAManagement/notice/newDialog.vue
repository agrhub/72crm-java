  ...
  <create-view :body-style="{height: '100%'}">
    <div
      ...
      <div
        slot="header"
        class="header">
        <span class="text">Thông báo mới</span>
        <img
            ...
            <template v-else-if="item.type =='plus'">
              <members-dep
                ...
                <flexbox
                  ...
                  <div class="add-item">+Thêm mới</div>
                </flexbox>
              </members-dep>
            </template>
            <el-input
  ...
  data() {
    ...
    return {
      formList: [
        ...
        { label: 'Phòng ban', field: 'dep', type: 'plus' },
        { label: 'Ngay bắt đầu', field: 'startTime', type: 'date' },
        { label: 'Ngày kết thúc', field: 'endTime', type: 'date' },
      ...
      rules: {
        ...
        content: [
          { required: true, message: 'Nội dung không được trống', trigger: 'blur' }
        ],
        startTime: [
          { required: true, message: 'Phải chọn ngày', trigger: 'blur' },
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          noticeAdd({
            ...
            .then(res => {
              this.$message.success('Thông báo mới đã gởi thành công')
            ...
            .catch(() => {
              this.$message.error('Không gởi được thông báo mới')
              this.loading = false
            })
        } else {
          return false
