      ...
      <flexbox class="se-section">
        ...
        <el-dropdown
          ...
          <flexbox class="se-select">
            <div class="se-select-name">{{ followType ? followType : 'Vui lòng chọn' }}</div>
            <i
        ...
        <el-checkbox
          ...
          v-model="isEvent">Thêm vào lời nhắc chương trình làm việc</el-checkbox>
        <el-button
          class="se-send"
          type="primary"
...
export default {
  ...
  computed: {
    logTypes() {
      if (this.oa) {
        return [
          { type: 'record', name: 'Bản ghi' },
          { type: 'log', name: 'Nhật ký' },
          { type: 'examine', name: 'Để xuất' },
          { type: 'task', name: 'Công việc' },
          { type: 'schedule', name: 'Lịch biểu' }
        ]
      } else {
        return [{ type: 'record', name: 'Bản ghi' }]
      }
    },
