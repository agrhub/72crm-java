      ...
      <div
        ...
        <flexbox
          ...
          <a
            ...
            <el-popover
              :title="item.name"
              :content="'Tỉ lệ thành công ' + item.rate + '%'"
              placement="bottom"
              width="150"
              trigger="hover">
              <div
...
export default {
  ...
  data() {
    return {
      ...
      headDetails: [
        ...
        { title: 'Giá trị cơ hội (đồng)', value: '' },
        { title: 'Tình trạng cơ hội', value: '' },
  ...
  computed: {
    ...
    tabnames() {
      ...
      if (this.crm.contacts && this.crm.contacts.index) {
        tempsTabs.push({ label: 'Liên hệ', name: 'contacts' })
      }

      if (this.crm.contract && this.crm.contract.index) {
        tempsTabs.push({ label: 'Hợp đồng', name: 'contract' })
      }
  ...
  methods: {
    ...
    handleBusinessStatus(isEnd, statusId, statusList, statusRemark) {
      ...
      if (statusList && statusList.length > 0) {
        ...
        if (isEnd == 0) {
          ...
          overItem.name = 'Thảnh công'
          overItem.title = 'Thảnh công' // 详情标题 和 内容
          overItem.detail = 'Tỉ lệ chiến thắng 100%'
          overItem['overIcon'] = ['el-icon-check', 'el-icon--right']
          overItem['class'] = 'state-suc'
        } else if (isEnd == 2) {
          overItem.name = 'Thất bại'
          overItem.title = 'Tỉ lệ chiến thắng 0%'
          overItem.detail = statusRemark
          overItem['overIcon'] = ['el-icon-circle-close', 'el-icon--right']
          overItem['class'] = 'state-fail'
        } else if (isEnd == 3) {
          overItem.name = 'Không hợp lệ'
          overItem.title = 'Tỉ lệ chiến thắng 0%'
          overItem.detail = statusRemark
    ...
    handleStatuChange(item, index) {
      ...
      if (!item.isdoing) {
        var message = 'Chắc chắn thực hiện ' + item.name
    ...
    handleStatuResult(item, index) {
      ...
      if (item.type == 2 || item.type == 3) {
        var message = 'Vui lòng điền ' + item.name + ' nguyên nhân：'
        var title = item.name + 'nguyên nhân'
        ...
        var message = 'Đảm bảo thực hiện cơ hội kinh doanh ' + item.name + '?'
        this.$confirm(message, 'Thông báo', {
          confirmButtonText: 'OK',
          cancelButtonText: 'Cancel',
          type: 'warning'
