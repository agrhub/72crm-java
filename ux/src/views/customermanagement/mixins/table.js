...
export default {
  ...
  methods: {
          ...
          if (this.crmType === 'customer') {
            this.list = res.data.list.map(element => {
              if (element.dealStatus === 1) {
                element.dealStatus = 'Chốt deal'
              } else {
                element.dealStatus = 'Chưa chốt'
              }
        ...
        filedGetTableField({
          ...
          .then(res => {
            for (let index = 0; index < res.data.length; index++) {
              ...
              // re calc width based on name length
              if (element.name) {
    ...
    exportInfos() {
      ...
      const loading = Loading.service({ fullscreen: true, text: 'Xuất báo cáo...' })
      request(params)
        .then(res => {
