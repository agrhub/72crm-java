    ...
    <flexbox
      ...
      <div class="handle-bar">
        ...
        <el-button
          ...
          class="handle-button"
          type="primary"
          @click.native="saveField(true)">Lưu và tạo một liên hệ mới</el-button>
        <el-button
          v-if="showDraft"
          class="handle-button"
          type="primary"
          plain
          @click.native="saveDraftField()">Lưu nháp</el-button>
        <el-button
          class="handle-button"
          type="primary"
...
export default {
  ...
  methods: {
      ...
      if (this.crmType == 'business' && item.data.formType == 'business_type') {
        ...
        if (item.data.formType == 'customer') {
          ...
          this.crmForm.crmFields.forEach(item => {
            ...
            } else if (item.key == 'Phương thức thanh toán') {
        ...
        var validateUnique = (rule, value, callback) => {
          if ((isArray(value) && value.length == 0) || !value) {
            ...
            filedValidates(validatesParams)
              ...
              .catch(error => {
                callback(new Error(error.msg ? error.msg : 'Lỗi định dạng'))
              })
          }
        }
        tempList.push({
    ...
    getTitle() {
      if (this.crmType == 'leads') {
        return this.action.type == 'update' ? 'Chỉnh sửa' : 'Tạo mới'
      } else if (this.crmType == 'customer') {
        return this.action.type == 'update' ? 'Chỉnh sửa' : 'Tạo mới'
      } else if (this.crmType == 'contacts') {
        return this.action.type == 'update' ? 'Chỉnh sửa' : 'Tạo mới'
