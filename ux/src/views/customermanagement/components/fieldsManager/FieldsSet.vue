<template>
  <el-dialog
    ...
    width="600px"
    @close="handleCancel">
    <div class="scene-name">Bạn có thể sắp xếp cột bằng cách kéo và thả</div>
    <flexbox class="scene-list">
      <div class="scene-list-box">
        <flexbox class="scene-list-head">
          ...
          <div class="scene-list-head-name">Cột được hiển thị</div>
      ...
      <div class="scene-list-box">
        <flexbox class="scene-list-head">
          ...
          <div class="scene-list-head-name">Cột ẩn</div>
  ...
  methods: {
    ...
    handleConfirm() {
      if (this.checkedLeftData.length < 2) {
        this.$message.error('Ít nhất hai cột phải được hiển thị')
      } else {
        crmFieldConfig({
          label: this.isSeas ? crmTypeModel.pool : crmTypeModel[this.crmType],
