<template>
  <el-dialog
    ...
    width="900px"
    @close="handleCancel">
    <div style="margin-bottom: 10px;">Điều kiện lọc</div>
    <el-form
      id="filter-container"
          ...
          <el-row :key="index">
            ...
            <el-col
              ...
              <el-select
                ...
                placeholder="Vui lòng chọn điều kiện lọc">
                <el-option
            ...
            <el-col :span="formItem.formType === 'datetime' || formItem.formType === 'date' || formItem.formType === 'map_address' ? 13 : 8">
              ...
              <el-input
                v-else
                v-model="formItem.value"
                placeholder="Vui lòng nhập tiêu chí bộ lọc"/>
            </el-col>
            <el-col
              :span="1"
              class="delete">
    ...
    <p
      ...
      <span class="desc">Có các sự trùng lắp trong bộ lọc!</span>
    </p>
    <el-button
      type="text"
    ...
    <div
      v-if="!isSeas"
      class="save">
      <el-checkbox v-model="saveChecked">Lưu kịch bản</el-checkbox>
      <el-input
        v-show="saveChecked"
...
export default {
  ...
  methods: {
    ...
    calConditionOptions(formType, item) {
      if (
        ...
        return [
          { value: 'is', label: 'bằng', disabled: false },
          { value: 'isNot', label: 'không bằng', disabled: false },
          { value: 'contains', label: 'chứa', disabled: false },
        ...
        return [
          ...
          { value: 'contains', label: 'chứa', disabled: false },
          { value: 'notContains', label: 'không chứa', disabled: false },
          { value: 'isNull', label: 'rỗng', disabled: false },
        ...
        return [
          ...
          { value: 'isnot', label: 'không bằng', disabled: false },
          { value: 'contains', label: 'chứa', disabled: false },
          { value: 'not_contain', label: 'không chứa', disabled: false }
        ]
      } else {
        return [
          { value: 'is', label: 'bằng', disabled: false },
          ...
          { value: 'contains', label: 'chứa', disabled: false },
          { value: 'notContains', label: 'không chứa', disabled: false },
          { value: 'startWith', label: 'bắt đầu từ ngày', disabled: false },
          { value: 'endWith', label: 'kết thức vào ngày', disabled: false },
          { value: 'isNull', label: 'rỗng', disabled: false },
          { value: 'isNotNull', label: 'không rỗng', disabled: false },
          { value: 'gt', label: 'lớn hơn', disabled: false },
          { value: 'egt', label: 'lớn hơn hoặc bằng', disabled: false },
    ...
    handleConfirm() {
      ...
      if (this.saveChecked) {
        if (!this.saveName || this.saveName === '') {
          this.$message.error('Tên kịch bảng không được để trống!')
          return
        }
      }
      for (let i = 0; i < this.form.length; i++) {
        ...
        if (o.formType == 'business_type') {
          ...
          if (!o.address.state && !o.address.city && !o.address.area) {
            this.$message.error('Vui lòng chọn giá trị của tiêu chí bộ lọc!')
            return
          }
        } else if (
          o.formType == 'date' ||
     ...
     */
    handleDelete(index) {
      this.$confirm('Bạn có chắc chắn muốn xóa phần dữ liệu này không?', 'Thông báo', {
