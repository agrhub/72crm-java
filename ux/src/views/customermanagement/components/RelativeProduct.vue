<template>
  <div
    ...
    <flexbox class="handle-footer">
      <div class="discount-title">Giảm giá toàn bộ đơn hàng（%）：<span class="discount-title-value">{{ totalInfo.discountRate }}</span></div>
      <div class="total-info">Sản phẩm được chọn：<span class="info-yellow">{{ list.length }}</span>&nbsp;sản phẩm&nbsp;&nbsp;Tổng cộng：<span class="info-yellow">{{ totalInfo.money }}</span>&nbsp;đồng</div>
    </flexbox>
...
export default {
  ...
  methods: {
    getFieldList() {
      ...
      this.fieldList.push({ prop: 'unit', width: '200', label: 'Đơn vị' })
      this.fieldList.push({ prop: 'price', width: '200', label: 'Giá bán' })
      this.fieldList.push({ prop: 'salesPrice', width: '200', label: 'Giá khuyến mãi' })
      this.fieldList.push({ prop: 'num', width: '200', label: 'Số lượng' })
      this.fieldList.push({
        prop: 'discount',
        width: '200',
        label: 'Giảm giá（%）'
      })
      this.fieldList.push({ prop: 'subtotal', width: '200', label: 'Tổng cộng' })
    },
    getDetail() {
      this.loading = true
