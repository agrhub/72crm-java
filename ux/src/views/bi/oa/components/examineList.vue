<template>
  <slide-view
    ...
    <div
      ...
      <span class="t-des">({{ name }}Số đề xuất：<span class="t-value">{{ totalCount }}lượt</span></span>
      <span
        v-if="showDes"
    ...
    <examine-section
      ...
      <p
        ...
        <el-button
          ...
          type="text">{{ loadMoreLoading ? 'Tải thêm' : 'Không có dữ liệu' }}</el-button>
      </p>
    </examine-section>
  </slide-view>
</template>
  ...
  computed: {
    ...
    desInfo() {
      if (this.params.categoryId == 2) {
        ...
        return 'Tổng số ngày đi công tác'
      } else if (this.params.categoryId == 4) {
        return 'Tổng số ngày làm thêm'
      } else if (this.params.categoryId == 5) {
        return 'Tổng số tiền hoàn lại'
      } else if (this.params.categoryId == 6) {
        return 'Tổng số tiền vay'
      }
      return ''
    },
    // 说明单位
    ...
    getList() {
      ...
      this.request({ page: this.page, limit: 15, ...this.params })
        .then(res => {
          ...
          this.sumData = this.desUnit == 'ngày' ? res.data.duration : res.data.money
