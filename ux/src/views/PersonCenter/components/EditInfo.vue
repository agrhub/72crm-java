<template>
  ...
  <el-dialog
    ...
    <el-form
      ...
      <el-form-item
        ...
        <el-select
          ...
          placeholder="Chọn">
          <el-option
            v-for="item in [{ label: 'Nam', value: 1 }, { label: 'Nữ', value: 2 }]"
            :key="item.value"
            :label="item.label"
            :value="item.value"/>
        </el-select>
    ...
    <span
      ...
      <el-button @click="hiddenView">Hủy</el-button>
    </span>
  </el-dialog>
</template>
  ...
  data() {
    /* var validateCRMMobile = (rule, value, callback) => {
      if (!value || value == '') {
        ...
        callback(new Error('Sai định dạng'))
      }
    } */
    var validateCRMEmail = (rule, value, callback) => {
      if (!value || value == '' || regexIsCRMEmail(value)) {
        ...
        callback(new Error('Sai định dạng email'))
      }
    }
    return {
      loading: false,
      showDialog: false,
      rules: {
        realname: [{ required: true, message: 'Họ và tên không được trống', trigger: 'blur' }],
