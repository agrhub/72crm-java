<template>
  <div
    ...
    <div class="header-handle">
      <el-button
        type="primary"
        @click="editClick('password')">Đổi mật khẩu</el-button>
      <el-button
        type="primary"
        @click="editClick('info')">Cập nhật thông tin</el-button>
      <el-button
        plain
        @click="editClick('back')">Đóng</el-button>
    </div>
    <flexbox class="person-info">
      <div class="person-head">
        ...
        <div
          ...
          <div>Nhấp để thay đổi hình đại diện</div>
        </div>
      </div>
      <div class="person-body">
        <div class="person-name">{{ userInfo.realname }}</div>
        <div class="person-detail">Phòng ban: {{ userInfo.deptName }}&nbsp;&nbsp;&nbsp;&nbsp;Chức vụ: {{ userInfo.post }}</div>
      </div>
    </flexbox>
      ...
      <flexbox
        ...
        <flexbox-item
          ...
          <flexbox
            ...
            <div class="info-cell-box-value">{{ item.props!=='sex'?userInfo[item.props]:userInfo[item.props]===1?'Nam':userInfo[item.props]===2?'Nữ':"" }}</div>
          </flexbox>
        </flexbox-item>
      </flexbox>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      list: [
        { name: 'Họ và tên', props: 'realname' },
        { name: 'Giới tính', props: 'sex' },
        { name: 'Tên đăng nhập', props: 'username' },
        { name: 'Email', props: 'email' },
        { name: 'Phòng ban', props: 'deptName' },
        { name: 'Chức vụ', props: 'post' },
        { name: 'Người quản lý', props: 'parentName' }
      ]
    }
  },
  computed: {
