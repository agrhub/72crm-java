...
export default {
  ...
  methods: {
    ...
    editBtn(val) {
      ...
      this.newTagTitle = 'Chỉnh sửa nhãn'
      this.tagContent = 3
    ...
    tagCreateSubmit(val, color) {
      const _this = this
      if (this.newTagTitle == 'Tạo nhãn mới') {
        workTasklableSaveAPI({
          name: val,
          color: color
        }).then(res => {
    ...
    back() {
      ...
      } else if (this.newTagTitle == 'Chỉnh sửa nhãn' && this.tagContent == 3) {
        this.tagContent = 2
