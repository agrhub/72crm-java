<template>
  <div class="employee-dep-management">
    <p class="title"> Quản lý nhân viên và phòng ban </p>
      ...
      <div class="system-view-table flex-index">
        <div
          ...
          <el-button
            ...
            type="primary"
            class="rt"
            @click="newBtn">Thêm nhân viên mới</el-button>
        </div>
        <flexbox
          v-if="selectionList.length > 0"
          class="selection-bar">
    ...
    <el-dialog
      ...
      title="Khôi phục tài khoản đăng nhập"
      width="30%">
      <div class="el-password">
        <el-form
    ...
    <el-dialog
      ...
      width="60%">
      <p class="new-dialog-title">Thông tin cơ bản</p>
      <el-form
        ref="dialogRef"
        :inline="true"
        :model="formInline"
        ...
        <el-form-item
          ...
          <el-input
            v-else
            v-model="formInline[item.field]"
            :disabled="dialogTitle == 'Chỉnh sửa thông tin nhân viên' && item.field == 'username'"/>
        </el-form-item>
      </el-form>
      <span
        slot="footer"
        ...
        <el-button @click="employeeCreateDialog = false">Đóng</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
var validateUsername = (rule, value, callback) => {
  ...
  /* else if (value && /^1\d{10}$/.test(value)) {
    ...
    callback(new Error('Hiện tại chỉ hỗ trợ số điện thoại di động ở Trung Quốc đại lục'))
  } */
}
...
export default {
  ...
  data() {
    return {
      ...
      /** 列表 */
      fieldList: [
        { field: 'username', value: 'Tên đăng nhập', width: '150' },
      ...
      dialogRules: {
        realname: [
          { required: true, message: 'Họ tên không để trống', trigger: 'blur' }
        ],
        password: [
          { required: true, message: 'Xin vui lòng nhập mật khẩu', trigger: 'blur' },
          { min: 6, max: 12, message: 'Độ dài từ 6 đến 12 ký tự', trigger: 'blur' }
    ...
    tableList: function() {
      if (this.dialogTitle === 'Nhân viên mới') {
        return [
          ...
          { field: 'parentId', value: 'Cấp trên trực tiếp', type: 'select' },
          { field: 'roleId', value: 'Vai trò', type: 'selectCheckout' }
        ]
      } else {
        return [
          {
            field: 'username',
            value: 'Tên đăng nhập',
            tips: 'Nếu bạn cần sửa đổi tên đăng nhập, vui lòng liên hệ quản trị viên'
          },
          { field: 'realname', value: 'Họ và tên' },
          { field: 'sex', value: 'Giới tính', type: 'select' },
          { field: 'email', value: 'Email' },
          { field: 'deptId', value: 'Phòng ban', type: 'select' },
          { field: 'post', value: 'Chức vụ' },
          { field: 'parentId', value: 'Cấp trên', type: 'select' },
          { field: 'roleId', value: 'Vai trò', type: 'selectCheckout' }
        ]
      }
    }
  ...
  methods: {
    ...
    append(data) {
      this.treeInput = ''
      this.labelName = 'Phòng ban mới'
      this.navBtnTitle = 'Phòng ban mới'
    ...
    edit(node, data) {
      ...
      this.navBtnTitle = 'Chỉnh sửa phòng ban'
      this.labelName = 'Chỉnh sửa phòng ban'
      this.getStructuresListBySuperior({ id: data.id, type: 'update' })
      this.depCreateDialog = true
    },
    // 删除组织架构
    remove(node, data) {
      this.$confirm('Xóa vĩnh viễn dữ liệu, bạn muốn tiếp tục?', 'Thông tin', {
    ...
    // 新增或编辑确定按钮
    submitDialog() {
      if (this.labelName == 'Phòng ban mới') {
    ...
    selectionBarClick(type) {
      ...
      if (type === 'lock' || type === 'unlock') {
        ...
        this.$confirm('Tài khoản nhân viên này sẽ được ' + message + ', bạn muốn tiếp tục?', 'Thông tin', {
          confirmButtonText: 'OK',
          cancelButtonText: 'Cancel',
          type: 'warning'
        })
