  ...
  <create-view
    ...
    <flexbox
          ...
          <div style="padding: 0 20px; font-size: 12px;">
            ...
            <div class="examine-items">
              <flexbox
                ...
                :key="index"
                class="examine-item">
                <div class="examine-item-name">Đầu tiên {{ index+1 | numberToZh }} cấp độ</div>
                <el-select
                  v-model="item.type"
            ...
            <div class="examine-add-des">
              <p><span class="examine-add-required">*</span>Khi chọn 'người phụ trách' để phê duyệt. Hệ thống sẽ chỉ thông báo cho người phụ trách.</p>
              ...
              <p><span class="examine-add-required">*</span>Khi bạn chọn 'Người dùng được chỉ định (Bất kỳ ai)', điều đó có nghĩa là bất kỳ ai trong số những người dùng được chỉ định đều có thể phê duyệt. Khi chọn 'Người dùng được chỉ định (Nhiều người được ký)', có nghĩa là tất cả những người dùng được chỉ định phải phê duyệt.</p>
            </div>
            <el-radio
              v-model="examineType"
      ...
      <div
        ...
        <el-button
          class="handle-button"
          @click.native="hidenView">Hủy bỏ</el-button>
        <el-button
          class="handle-button"
          type="primary"
          @click.native="nextPage">Trang tiếp theo</el-button>
      </div>
      <div
        v-if="currentPage == 2"
        class="handle-bar">
        ...
        <el-button
          class="handle-button"
          type="primary"
          @click.native="saveField">Lưu lại</el-button>
        <el-button
          class="handle-button"
          type="primary"
      ...
      examineList: [
        {
          ...
          show: false,
          options: [
            { name: 'Người phụ trách', value: 1 },
            { name: 'Người dùng được chỉ định (bất kỳ người nào)', value: 2 },
            { name: 'Người dùng được chỉ định (nhiều người cùng ký)', value: 3 }
          ]
        }
      ]
    }
  ...
  computed: {
    title() {
      if (this.handle.action === 'save') {
        return 'Loại phê duyệt mới'
      } else if (this.handle.action === 'update') {
        return 'Chỉnh sửa loại phê duyệt'
      }
    }
  },
  mounted() {
        ...
        for (let index = 0; index < this.handle.data.stepList.length; index++) {
          ...
          if (index === 0) {
            ...
            item['options'] = [
              ...
              { name: 'Người dùng được chỉ định (bất kỳ người nào)', value: 2 },
              { name: 'Người dùng được chỉ định (nhiều người cùng ký)', value: 3 },
              { name: 'Người giám sát của người phê duyệt cấp trên', value: 4 }
            ]
          }
  ...
  methods: {
    ...
    getField() {
      ...
      field.push({
        ...
        name: 'Phòng ban',
        setting: [],
        inputTips: 'Mặc định toàn công ty',
        value: {
    ...
    submiteParams(array) {
      ...
      oaExamineCategorySave(params)
        .then(res => {
          ...
          if (this.handle.action == 'save') {
            // 如果是新建 提示去创建表单
            this.$confirm('Bạn muốn tiếp tục thực hiện việc tạo biểu mẫu phê duyệt?', 'Thông báo', {
    ...
    selectOptionsFocus(item, index) {
      if (this.examineList.length > 1) {
        ...
        var options = [{ name: 'Người phụ trách', value: 1 }]

        if (!removeTwo) {
          options.push({ name: 'Người dùng được chỉ định (bất kỳ người nào)', value: 2 })
        }
        if (!removeThree) {
          options.push({ name: 'Người dùng được chỉ định (nhiều người cùng ký)', value: 3 })
        }
        if (!removeFour) {
          options.push({ name: 'Người giám sát của người phê duyệt cấp trên', value: 4 })
        }
