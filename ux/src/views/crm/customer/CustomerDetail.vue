  ...
  computed: {
    ...
    tabnames() {
      ...
      if (this.crm.contacts && this.crm.contacts.index) {
        tempsTabs.push({ label: 'Thông tin liên hệ', name: 'contacts' })
      }
      tempsTabs.push({ label: 'Đội nhóm', name: 'team' })
      if (this.crm.business && this.crm.business.index) {
        tempsTabs.push({ label: 'Mối quan hệ kinh doanh', name: 'business' })
      }
      ...
      if (this.crm.receivables && this.crm.receivables.index) {
        tempsTabs.push({ label: 'Thông tin thanh toán', name: 'returnedmoney' })
      }
      tempsTabs.push({ label: 'Đính kèm', name: 'file' })
      tempsTabs.push({ label: 'Lịch sử', name: 'operationlog' })
      return tempsTabs
    },
    /**
    ...
    getDetial() {
      ...
      crmCustomerRead({
        ...
        .then(res => {
          ...
          this.headDetails[1].value = res.data.dealStatus == 1 ? 'Chốt deal' : 'Chưa bán'
