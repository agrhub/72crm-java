  ...
  <div
    ...
    <flexbox
      ...
      <el-button
        class="rc-head-item"
        type="primary"
        @click.native="createClick">Thêm cơ hội kinh doanh mới</el-button>
      <el-button
        v-if="canRelation"
        class="rc-head-item"
        type="primary"
        @click.native="unRelevanceHandleClick">Xóa cơ hội kinh doanh</el-button>
      <el-popover
        v-if="canRelation"
        ...
        <el-button
          ...
          type="primary"
          @click.native="showRelativeView = true">Xem</el-button>
      </el-popover>
  ...
  methods: {
    ...
    unRelevanceHandleClick() {
      if (this.selectionList.length == 0) {
        this.$message.error('Chưa chọn cơ hội kinh doanh')
      } else {
        this.$confirm('Xóa cơ hội kinh doanh?', 'Thông báo', {
    ...
    getFieldList() {
      ...
      this.fieldList.push({ prop: 'statusName', width: '200', label: 'Trạng thái' })
    },
