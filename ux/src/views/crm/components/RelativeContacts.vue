  ...
  <div
    ...
    <flexbox
      ...
      <el-button
        class="rc-head-item"
        type="primary"
        @click.native="createClick">Thêm liên lạc mới</el-button>
      <el-button
        v-if="canRelation"
        class="rc-head-item"
        type="primary"
        @click.native="unRelevanceHandleClick">Xóa liên lạc</el-button>
      <el-popover
        v-if="canRelation"
        ...
        <el-button
          ...
          type="primary"
          @click.native="showRelativeView = true">Quan hệ</el-button>
      </el-popover>
  ...
  methods: {
    ...
    unRelevanceHandleClick() {
      if (this.selectionList.length == 0) {
        this.$message.error('Chưa chọn liên lạc')
      } else {
        this.$confirm('Xác nhận xóa?', 'Thông báo', {
          ...
          .catch(() => {
            this.$message.info('Không thành công')
          })
      }
    },
    ...
    getFieldList() {
      ...
      this.fieldList.push({ prop: 'mobile', width: '200', label: 'Số điện thoại di động' })
      this.fieldList.push({ prop: 'post', width: '200', label: 'Số điện thoại bàn' })
    },
