  ...
  <div>
    <flexbox class="user-container">
        ...
        <flexbox class="user-info">
          ...
          <members-dep
            ...
            <el-button
              slot="membersDep"
              type="text"
              class="user-switch">Chuyển phòng ban</el-button>
          </members-dep>
      ...
      <el-button
        ...
        @click="showDuplicateCheck = true">Kiểm tra dữ liệu trùng lấp</el-button>
    </flexbox>
...
export default {
  ...
  data() {
    return {
      ...
      // 时间值
      timeTypeValue: { label: 'Quý này', value: 'quarter' },
      // 展示查重
    ...
    popoverSubmit(users, strucs) {
      ...
      if (this.users.length === 1 && this.strucs.length === 0) {
        ...
        if (this.users.length > 0) {
          name = this.users.length + ' nhân viên'
        }
        if (this.strucs.length > 0) {
          if (this.users.length > 0) {
            name = name + ','
          }
          name = name + this.strucs.length + ' phòng ban'
        }
        this.filtersInfo = {
          realname: name,
