<template>
  <div class="journal oa-bgcolor">
    <el-button
      type="primary"
      class="new-btn"
      @click="newBtn">Viết báo cáo</el-button>
    <el-tabs
      v-model="activeName"
      @tab-click="tabClick">
      <el-tab-pane
...
export default {
  ...
  data() {
    return {
      ...
      tabsData: [
        { label: 'Tất cả', key: '1' },
        { label: 'Báo cáo bạn đã gửi', key: '2' },
        { label: 'Báo cáo bạn đã nhận được', key: '3' },
        { label: 'Chưa đọc', key: '4' }
      ],
      // 日志数据
      journalData: [],
      // 显示新建页面
  ...
  methods: {
      ...
      journalList(params)
        .then(res => {
          ...
          if (res.data.lastPage === true) {
            this.loadText = 'Không có dữ liệu'
            this.loadMoreLoading = false
          } else {
            this.loadText = 'Tải thêm'
    ...
    submitBtn(key, batchId, img, relevanceAll) {
      ...
      if (this.dialogTitle == 'Viết báo cáo') {
        // 图片
        const pramas = {
          categoryId: key || '',
    ...
    // 编辑按钮
    editBtn(val) {
      this.dialogTitle = 'Chỉnh sửa báo cáo'
      this.formData = val
