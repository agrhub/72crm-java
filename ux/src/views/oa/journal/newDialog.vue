...
export default {
  ...
  data() {
    return {
      activeName: '1',
      tabsData: [
        { label: 'Hằng ngày', key: '1' },
        { label: 'Hàng tuần', key: '2' },
        { label: 'Báo cáo hàng tháng', key: '3' }
      ],
      // 表格数据
      formList: [],
      dateList: [
        { label: 'Nội dung công việc hôm nay', model: 'content' },
        { label: 'Nội dung công việc ngày mai', model: 'tomorrow' },
        { label: 'Những vấn đề gặp phải', model: 'question' }
      ],
      weekList: [
        { label: 'Nội dung công việc tuần này', model: 'content' },
        { label: 'Nội dung công việc tuần tiếp theo', model: 'tomorrow' },
        { label: 'Những vấn đề gặp phải', model: 'question' }
      ],
      monthList: [
        { label: 'Nội dung công việc tháng này', model: 'content' },
        { label: 'Nội dung công việc tháng tiếp theo', model: 'tomorrow' },
        { label: 'Những vấn đề gặp phải', model: 'question' }
      ],
      batchId: guid(),
      imageFileList: [],
  ...
  mounted() {
    ...
    if (this.formData.categoryId) {
      switch (this.formData.categoryId) {
        case 1:
          this.tabsData = [{ label: 'Hằng ngày', key: '1' }]
          this.formList = this.dateList
          break
        case 2:
          this.tabsData = [{ label: 'Hằng tuần', key: '1' }]
          this.formList = this.weekList
          break
        case 3:
          this.tabsData = [{ label: 'Hằng tháng', key: '1' }]
          this.formList = this.monthList
          break
      }
    }

    if (this.dialogTitle != 'Viết báo cáo' && this.formData.batchId) {
  ...
  methods: {
    ...
    submitBtn() {
      if (
        ...
        this.$message.error('Điền vào ít nhất một mục')
      }
    },
    // 图片和附件
    // 上传图片
