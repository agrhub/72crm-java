...
export default {
  ...
  computed: {
    statusOptions() {
      if (this.by == 'examine') {
        return [
          { label: 'Chờ đợi sự chấp thuận của bạn', key: '1' },
          { label: 'Bạn đã chấp thuận', key: '2' }
        ] // 1待我审批的、2我已审批的
      } else {
        return [
          { label: 'Tất cả', key: '' },
          { label: 'Đang chờ xử lý', key: '0' },
          { label: 'Đang xem xét', key: '3' },
          { label: 'Đã duyệt', key: '1' },
          { label: 'Từ chối', key: '2' },
          { label: 'Rút lại', key: '4' }
        ]
      }
