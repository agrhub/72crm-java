  ...
  <div class="journal oa-bgcolor">
    <div class="btn-select">
      <div class="select-group">
        <label>Danh mục phê duyệt</label>
        <el-select
      ...
      <el-button
        type="primary"
        class="new-btn"
        @click="newBtn">Thêm mới</el-button>
    </div>
    <el-tabs
      v-model="activeName"
      @tab-click="tabClick">
...
export default {
  ...
  data() {
    return {
      ...
      activeName: 'my',
      tabsData: [
        { label: 'Bạn đã tạo', key: 'my' },
        { label: 'Bạn đồng ý', key: 'examine' }
      ],
      // 审批类型
      categoryType: '',
      categoryOptions: [],
  ...
  methods: {
    ...
    getExamineCategoryList() {
      ...
      oaExamineCategoryList()
        .then(res => {
          this.loading = false
          this.categoryOptions = [{ categoryId: '', title: 'Tất cả' }].concat(
            res.data
          )
        })
        .catch(() => {
