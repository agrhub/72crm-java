#namespace("crm.product")
    ...
    #sql("queryProductPageList")
        SELECT *,
        (select value from `72crm_admin_fieldv` a  where a.batch_id = c.batch_id and a.name = 'product_unit') as productUnit,
        (select value from `72crm_admin_fieldv` b  where b.batch_id = c.batch_id and b.name = 'product_status') as productStatus
        from
