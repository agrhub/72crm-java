...
public class CrmReceivablesService {
    ...
    public R saveOrUpdate(JSONObject jsonObject) {
        ...
        if (crmReceivables.getReceivablesId() == null) {
            ...
            if (count != null && count > 0) {
                return R.error("The payment number already exists, please proofread and add it！");
            }
    ...
    public List<Record> information(Integer id) {
        ...
        recordList.add(new Record().set("name","Contract No").set("value",new Record().set("contactId",record.getInt("contract_id")).set("contractNum",record.getStr("contract_num"))).set("formType","contract").set("field_type",1));
