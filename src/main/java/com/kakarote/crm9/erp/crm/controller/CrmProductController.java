...
public class CrmProductController extends Controller {
    ...
    @NotNullValidate(value = "productId", message = "Product id cannot be empty")
    public void queryById(@Para("productId") Integer productId) {
        ...
        if (number == 0) {
            renderJson(R.error("Product deleted"));
            return;
        }
    ...
    @NotNullValidate(value = "productIds", message = "Product id cannot be empty")
    ...
    private void export(List<Record> recordList){
        try (ExcelWriter writer = ExcelUtil.getWriter()) {
            ...
            writer.merge(headList.size() - 1, "product information");
            ...
            Log.getLog(getClass()).error("Export product error：",e);
        }
    }
    ...
    public void downloadExcel() {
        ...
        HSSFSheet sheet = wb.createSheet("Product Import Form");
        ...
        font.setFontHeightInPoints((short)16);
        cellStyle.setFont(font);
        titleRow.createCell(0).setCellValue("Product import template (*) is required");
        ...
        try {
            ...
            for (int i = 0; i < recordList.size(); i++) {
                ...
                if ("product type".equals(record.getStr("name"))) {
