...
public class CrmCustomerController extends Controller {
    ...
    @NotNullValidate(value = "customerId", message = "Customer id cannot be empty")
     ...
     * 根据客户名称查询
     */
    @NotNullValidate(value = "name", message = "Customer name cannot be empty")
    ...
    @NotNullValidate(value = "customerIds", message = "Customer id cannot be empty")
    ...
    @NotNullValidate(value = "ids", message = "Customer id cannot be empty")
    @NotNullValidate(value = "isLock", message = "The lock state cannot be empty")
    ...
    @NotNullValidate(value = "customerIds", message = "Customer id cannot be empty")
    @NotNullValidate(value = "newOwnerUserId", message = "The new person in charge cannot be empty")
    @NotNullValidate(value = "transferType", message = "The removal method cannot be empty")
        ...
        Db.tx(()->{
            for (String customerId : customerIdsArr) {
                if(Db.queryInt("select count(*) from `72crm_crm_customer` where find_in_set(?,ro_user_id) and customer_id = ?",BaseUtil.getUserId(),customerId)>0){
                    renderJson(R.error("Permission denied"));
                    return false;
                }
    ...
    @NotNullValidate(value = "followupDay", message = "Follow-up days cannot be empty")
    @NotNullValidate(value = "dealDay", message = "Dealing days cannot be empty")
    @NotNullValidate(value = "type", message = "The enabled state cannot be empty")
     ...
     * 添加跟进记录
     */
    @NotNullValidate(value = "typesId", message = "Customer id cannot be empty")
    ...
    private void export(List<Record> recordList){
        ...
        try {
            ...
            writer.merge(headList.size()-1, "Customer Information");
            ...
            recordList.forEach(record -> {
                record.set("deal_status",record.getInt("deal_status").equals(1)?"Sold": "Unsold");
                list.add(record.getColumns());
            });
            ...
            Log.getLog(getClass()).error("Export client error:",e);
        } finally {
            // 关闭writer，释放内存
            writer.close();
        }
...
//    }
    ...
    public void downloadExcel() {
        ...
        HSSFSheet sheet = wb.createSheet("Customer Import Form");
        ...
        font.setFontHeightInPoints((short) 16);
        cellStyle.setFont(font);
        titleRow.createCell(0).setCellValue("Customer import template (*) is required");
    ...
    @NotNullValidate(value = "customerNum", message = "Please fill in the number of customers",type = HttpEnum.JSON)
