...
public class CrmProductService {
    ...
    public R saveAndUpdate(JSONObject jsonObject) {
        ...
        if (crmProduct.getProductId() == null) {
            ...
            if (product != 0){
                return R.error("The product number already exists, please proofread it before adding！");
            }
    ...
    public List<Record> information(Integer id) {
        ...
        recordList.add(new Record().set("name","Product type").set("value",new Record().set("categoryId",record.getInt("category_id")).set("categoryName",record.getStr("categoryName"))).set("formType","category").set("field_type",1));
    ...
    public R updateStatus(String ids, Integer status) {
        ...
        StringBuilder sqlfield = new StringBuilder("update 72crm_admin_fieldv set value = '" + a + "' where name = 'product_status' and batch_id in ( ");
    ...
    public R getCheckingField(){
        return R.ok().put("data","Product name");
    }
    /**
     * @author zxy
     * 获取上架商品
