...
public class CrmContractService {
    ...
    public List<Record> information(Integer id) {
        ...
        recordList.add(new Record().set("name","Opportunity name").set("value",new Record().set("businessId",record.getInt("business_id")).set("businessName",record.getStr("business_name"))).set("formType","business").set("field_type",1));
        recordList.add(new Record().set("name","Customer signatory").set("value",new Record().set("contacts_id",record.getInt("contacts_id")).set("contacts_name",record.getStr("contacts_name"))).set("formType","contacts").set("field_type",1));
        recordList.add(new Record().set("name","Company signatory").set("value",Db.template("admin.user.queryByIds",Kv.by("ids", StrUtil.isNotEmpty(record.getStr("company_user_id")) ? record.getStr("company_user_id") : 0)).find()).set("formType","user").set("field_type",1));
        ...
        recordList.forEach(record1 -> {
            ...
            if ("Order time".equals(name) || "Contract start time".equals(name) || "Contract end time".equals(name)){
    ...
    public R deleteByIds(String contractIds) {
        ...
        if (list.size() > 0) {
            return R.error("The data has been referenced by other modules and cannot be deleted！");
        }
    ...
    public R saveAndUpdate(JSONObject jsonObject) {
        ...
        if (crmContract.getContractId() == null) {
            ...
            if (contract != 0) {
                return R.error("The contract number already exists, please proofread it before adding！");
            }
            ...
            if (map.get("status") == 0) {
                return R.error("No review step started, cannot add！");
            } else {
            ...
            if (contract.getCheckStatus() != 4 && contract.getCheckStatus() != 2 && contract.getCheckStatus() != 5) {
                return R.error("Cannot edit, please withdraw and then edit！");
            }
    ...
    public R setContractConfig(Integer status, Integer contractDay) {
        if (status == 1 && contractDay == null) {
            return R.error("contractDay cannot be empty");
        }
