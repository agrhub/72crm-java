...
public class CrmCustomerService{
    ...
    public R addOrUpdate(JSONObject jsonObject, String type){
        ...
        if(crmCustomer.getCustomerId() != null){
            ...
            if(! queryCustomerSettingNum(1, crmCustomer.getOwnerUserId().longValue())){
                return R.error("The customer has reached the upper limit and cannot be added");
            }
    ...
    public List<Record> information(Integer customerId) {
        ...
        recordList.add(new Record().set("name","Deal status").set("value",record.getInt("deal_status") == 0? "Unsold": "Already dealt"). set("formType","deal_status").set("field_type",1));
        recordList.add(new Record().set("name","regional positioning").set("value",Kv.by("location", record.getStr("location"))
    ...
    public R lock(CrmCustomer crmCustomer){
        ...
        if(number > 0){
            return R.error("Customers who have completed transactions do not need to be locked");
        }
        if(! isMaxLock(crmCustomer.getIsLock(), ids)){
            return R.error("The number of employees that can be locked reaches the upper limit");
        }
        ...
        for(String id : idsArr){
            ...
            if (0 == dealStatus && 1 == customer.getDealStatus()){
                if(!queryCustomerSettingNum(1,Long.valueOf(customer.getOwnerUserId()))){
                    return R.error("A person in charge of the selected customers has reached the upper limit");
                }
            }
        }
    ...
    public R updateOwnerUserId(CrmCustomer crmCustomer){
        ...
        if(! isMaxOwner(crmCustomer.getNewOwnerUserId(), customerIdsArr)){
            return R.error("The employee has reached the maximum number of customers");
        }
    ...
    public List<Record> getMembers(Integer customerId){
        ...
        for(String memberId : memberIdsSet){
            ...
            if(roUserId.contains(memberId)){
                record.set("power", "Read only").set("groupRole", "Ordinary member");
            }
            if(rwUserId.contains(memberId)){
                record.set("power", "Read and write").set("groupRole", "Ordinary member");
            }
    ...
    public R updateCustomerByIds(String ids){
        ...
        if(count > 0){
            return R.error("The selected customers are locked and cannot be put into the open sea!");
        }
        ...
        for(String id : idsArr){
            if(!BaseUtil.getUserId().equals(BaseConstant.SUPER_ADMIN_USER_ID) && !BaseUtil.getUser().getRoles().contains(BaseConstant.SUPER_ADMIN_ROLE_ID) && Db.queryInt(Db.getSql("crm.customer.queryIsRoUser"), BaseUtil.getUserId(), id) > 0){
                return R.error("No permission to put");
            }
    ...
    public R getCheckingField(){
        return R.ok().put("data", "Client's name");
    }
    ...
    public R customerSetting(CrmCustomerSetting customerSetting){
        ...
        Db.tx(() -> {
            ...
            for (Integer deptId : customerSetting.getDeptList()) {
                ...
                if (count > 0) {
                    errorList.add("There are already employee or department information included in other rules");
                    return false;
                }
