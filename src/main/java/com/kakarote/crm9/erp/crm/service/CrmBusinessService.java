...
public class CrmBusinessService {
    ...
    public R addOrUpdate(JSONObject jsonObject) {
        ...
        if (crmBusiness.getBusinessId() != null) {
            if (!BaseUtil.getUserId().equals(BaseConstant.SUPER_ADMIN_USER_ID) && !BaseUtil.getUser().getRoles().contains(BaseConstant.SUPER_ADMIN_ROLE_ID) && Db.queryInt(Db.getSql("crm.business.queryIsRoUser"), BaseUtil.getUserId(), crmBusiness.getBusinessId()) > 0) {
                return R.error("Permission denied");
            }
    ...
    public List<Record> information(Integer busienssId) {
        ...
        recordList.add(new Record().set("name", "Opportunity stage").set("value", new Record().set("statusId", record.getInt("status_id")).set("statusName", record.getStr("statusName"))).set("formType", "statusName").set("field_type", 1));
        recordList.add(new Record().set("name", "Opportunity Status Group").set("value", new Record().set("typeId", record.getInt("type_id")).set("typeName", record.getStr("typeName"))).set("formType", "typeName").set("field_type", 1));
        recordList.add(new Record().set("name", "Client's name").set("value", new Record().set("customerId", record.getInt("customer_id")).set("customerName", record.getStr("customer_name"))).set("formType", "customer").set("field_type", 1));
    ...
    public R deleteByIds(String businessIds) {
        ...
        if (number > 0) {
            return R.error("This piece of data must be associated with other data, please do not delete");
        }
    ...
    public R transfer(CrmBusiness crmBusiness) {
        ...
        for (String businessId : businessIdsArr) {
            if (!BaseUtil.getUserId().equals(BaseConstant.SUPER_ADMIN_USER_ID) && !AuthUtil.isRwAuth(Integer.parseInt(businessId), "business")) {
                return R.error("Transfer without permission");
            }
    ...
    public List<Record> getMembers(Integer businessId) {
        ...
        if (crmBusiness.getOwnerUserId() != null) {
            ...
            recordList.add(ownerUser.set("power", "Person in charge authority").set("groupRole", "Principal"));
        }
        ...
        for (String memberId : memberIdsSet) {
            ...
            if (roUserId.contains(memberId)) {
                record.set("power", "read only").set("groupRole", "common member");
            }
            if (rwUserId.contains(memberId)) {
                record.set("power", "read and write").set("groupRole", "common member");
            }
            ...
            if (StrUtil.isNotEmpty(id)) {
                ...
                for (String memberId : memberArr) {
                    if (ownerUserId.equals(Long.valueOf(memberId))) {
                        return R.error("The person in charge cannot be repeatedly selected as a team member\n");
                    }
    ...
    public List<Record> queryField(Integer businessId) {
        ...
        fieldList.add(new Record().set("field_name", "type_id").set("name", "Opportunity Status Group").set("value", business.getInt("type_id")).set("form_type", "business_type").set("setting", new String[0]).set("is_null", 1).set("field_type", 1));
        fieldList.add(new Record().set("field_name", "status_id").set("name", "Opportunity stage").set("value", business.getInt("status_id")).set("form_type", "business_status").set("setting", new String[0]).set("is_null", 1).set("field_type", 1));
        ...
        fieldList.add(new Record().set("field_name", "product").set("name", "Product").set("value", kv).set("form_type", "product").set("setting", new String[]{}).set("is_null", 0).set("field_type", 1));
        return fieldList;
    }
    ...
    public List<Record> queryBusinessStatusOptions(String type) {
        ...
        for (Record record : businessTypeList) {
            ...
            if ("condition".equals(type)) {
                ...
                win.set("name", "Win").set("typeId", typeId).set("statusId", "win");
                businessStatusList.add(win);
                Record lose = new Record();
                lose.set("name", "Lose order").set("typeId", typeId).set("statusId", "lose");
                businessStatusList.add(lose);
                Record invalid = new Record();
                invalid.set("name", "Invalid").set("typeId", typeId).set("statusId", "invalid");
    ...
    public R addRecord(AdminRecord adminRecord) {
        ...
        if (StrUtil.isEmpty(adminRecord.getCategory())) {
            return R.error("Follow-up type cannot be empty");
        }
