...
public class AdminRoleController extends Controller {
     ...
     */
    @Permissions("manage:permission:update")
    @NotNullValidate(value = "roleId",message = "Role id cannot be empty")
    @NotNullValidate(value = "roleName",message = "Role name cannot be empty")
    public void update(@Para("")AdminRole adminRole){
        ...
        if (number > 0){
            renderJson(R.error("Role name already exists"));
        }else {
