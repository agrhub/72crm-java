...
public class AdminDeptService {
    public R setDept(AdminDept adminDept) {
        ...
        if (adminDept.getDeptId() == null) {
            ...
            if (adminDept.getPid() != null && adminDept.getPid() != 0) {
                ...
                if (!isContain) {
                    return R.error("The subordinate department of this department cannot be set as the superior department");
                }
            }
            bol = adminDept.update();
        }
        return R.isSuccess(bol,"Setup failed");
    }
    ...
    public R deleteDept(String id) {
        ...
        if(userCount>0){
            return R.error("There are employees in this department and cannot be deleted！");
        }
        Integer childDeptCount = Db.queryInt("select count(*) from 72crm_admin_dept where pid = ?", id);
        if(childDeptCount>0){
            return R.error("There are subordinate departments under this department, which cannot be deleted！");
        }
