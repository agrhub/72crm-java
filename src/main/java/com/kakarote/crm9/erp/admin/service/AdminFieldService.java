...
public class AdminFieldService {
    ...
    public List<Record> queryAddField(CrmEnum crmEnum){
        ...
        if (crmEnum == CrmEnum.CRM_CUSTOMER){
            ...
            fieldList.add(map.set("field_name", "map_address").set("name", "Regional positioning").set("form_type", "map_address").set("is_null", 0).set("field_type",FormTypeEnum.TEXT.getType()));
        }else if (crmEnum == CrmEnum.CRM_BUSINESS){
            fieldList.add(new Record().set("field_name","type_id").set("name","Opportunity Status Group").set("value","").set("form_type","business_type").set("setting",new String[0]).set("is_null",1).set("field_type",FormTypeEnum.TEXT.getType()));
            fieldList.add(new Record().set("field_name","status_id").set("name","Opportunity stage").set("value","").set("form_type","business_status").set("setting",new String[0]).set("is_null",1).set("field_type",FormTypeEnum.TEXT.getType()));
        ...
        if (crmEnum == CrmEnum.CRM_BUSINESS || crmEnum == CrmEnum.CRM_CONTRACT){
            ...
            fieldList.add(record.set("field_name","product").set("name","Product").set("value",Kv.by("discount_rate", "").set("product", new ArrayList<>()).set("total_price", "")).set("formType","product").set("setting",new String[]{}).set("is_null",0).set("field_type",FormTypeEnum.TEXT.getType()));
        }
        return fieldList;
    }
    ...
    public R save(JSONObject jsonObject) {
        ...
        Map<String, List<AdminField>> collect = adminFields.stream().map(adminField -> TypeUtils.castToJavaBean(adminField, AdminField.class)).collect(Collectors.groupingBy(AdminField::getFieldName));
        for (Map.Entry<String, List<AdminField>> entry : collect.entrySet()) {
            if (entry.getValue().size() > 1) {
                return R.error("Custom form name cannot be repeated！");
            }
        }
        ...
        if (categoryId != null && Db.queryInt("select ifnull(is_sys,0) from 72crm_oa_examine_category where category_id = ?", categoryId) == 1) {
            return R.error("System approval type currently does not support editing");
        }
        ...
        List<AdminField> fieldSorts = AdminField.dao.find("select field_name from 72crm_admin_field where label = ?", label);
        List<String> nameList = fieldSorts.stream().map(AdminField::getFieldName).collect(Collectors.toList());
        if (arr.size() > 0) {
    ...
    public R setFelidStyle(Kv kv) {
        ...
        return R.ok().put("data", "Edit successfully");
    }
    ...
    public List<Record> queryListHead(AdminFieldSort adminFieldSort) {
        ...
        if (0 == number) {
            ...
            if (CrmEnum.CRM_CUSTOMER.getType() == adminFieldSort.getLabel()){
                fieldUtil.add("deal_status", "Transaction status", 3)
                        .add("pool_day", "Days before entering the high seas", 5)
                        .add("last_time", "Last follow-up time", 4)
                        .add("last_content","Final follow-up record",1);
            }else if (CrmEnum.CRM_BUSINESS.getType() == adminFieldSort.getLabel()){
                fieldUtil.add("type_name", "Opportunity Status Group", 3).add("status_name", "Opportunity stage", 3);
            }else if (CrmEnum.CRM_CONTRACT.getType() == adminFieldSort.getLabel()){
                fieldUtil.add("received_money","Amount received",6).add("unreceived_money","Uncollected amount",6);
            }else if (CrmEnum.CRM_RECEIVABLES.getType() == adminFieldSort.getLabel()){
                fieldUtil.add("contract_money","Contract amount",6);
                ...
                fieldUtil.add("last_content","Final follow-up record",1);
            }
            fieldUtil.add("update_time", "Update time",4).add("create_time", "Creation time",4)
                    .add("owner_user_name", "Principal",1).add("create_user_name", "Creator",1);
            fieldUtil.getAdminFieldSortList().forEach(fieldSort -> {
                ...
                if ("customer_id".equals(fieldSort.getFieldName())){
                    fieldSort.setFieldName("customer_name");
                } else if ("category_id".equals(fieldSort.getFieldName())){
                    fieldSort.setFieldName("category_name");
                } else if ("contacts_id".equals(fieldSort.getFieldName())){
                    fieldSort.setFieldName("contacts_name");
                } else if ("company_user_id".equals(fieldSort.getFieldName())){
                    fieldSort.setFieldName("company_user_name");
                } else if ("business_id".equals(fieldSort.getFieldName())){
                    fieldSort.setFieldName("business_name");
                } else if ("contract_id".equals(fieldSort.getFieldName())){
                    fieldSort.setFieldName("contract_num");
                } else if ("plan_id".equals(fieldSort.getFieldName())){
                    fieldSort.setFieldName("plan_num");
                }
            });
    ...
    public R fieldConfig(AdminFieldSort adminFieldSort) {
        ...
        if (sortArr.length < 2) {
            return R.error("Show at least 2 columns");
        }
