...
public class AdminRoleService {
    ...
    public R save(AdminRole adminRole) {
        ...
        if (number > 0) {
            return R.error("Role name already exists");
        }
    ...
    public R relatedUser(AdminUserRole adminUserRole) {
        if (adminUserRole != null && adminUserRole.getUserIds() != null) {
            ...
            return R.error("Please select role and employee");
        }
    }
    ...
    public R unbindingUser(AdminUserRole adminUserRole) {
        if (adminUserRole.getUserId().equals(BaseConstant.SUPER_ADMIN_USER_ID)) {
            return R.error("Super administrator cannot be changed");
        }
        if(Db.queryInt("select count(1) from `72crm_admin_user_role` where user_id = ?",adminUserRole.getUserId())==1){
            return R.error("The user needs at least one role");
        }
    ...
    public R getRoleTypeList(){
        ...
        data.add(Kv.by("name","System management").set("roleType",1));
        data.add(Kv.by("name","Office Management").set("roleType",7));
        data.add(Kv.by("name","Customer Management").set("roleType",2));
        data.add(Kv.by("name","Project Management").set("roleType",8));
