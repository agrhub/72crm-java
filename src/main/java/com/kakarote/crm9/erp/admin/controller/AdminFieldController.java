...
public class AdminFieldController extends Controller {
     ...
     * 验证字段数据
     */
    @NotNullValidate(value = "value",message = "Field verification parameter error")
    @NotNullValidate(value = "fieldId",message = "Field verification parameter error")
    public void verify(@Para("fieldId") Integer fieldId,@Para("value") String value,@Para("batchId") String batchId){
        ...
        renderJson(R.isSuccess(num==0,adminField.getName()+"existed"));
    }
     ...
     */
    @NotNullValidate(value = "label",message = "label cannot be empty")
    @NotNullValidate(value = "noHideIds",message = "Display column cannot be empty")
    ...
    public void getCheckingField(@Para("type")Integer type){
        ...
        switch (type) {
            ...
            case 4:
                ...
                break;
            default:
                data = R.error("type does not meet the requirements");
        }
        renderJson(data);
    }
}
